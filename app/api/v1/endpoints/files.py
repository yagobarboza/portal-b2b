...
async def upload_file(
    ...
    user: User = Depends(require_permission("files.manage")),
) -> FileUploadResponse:
    ...
    url = storage.generate_signed_url(storage_key)
...
async def download_file(
    ...
    storage = StorageService()
    url = storage.generate_signed_url(file_obj.storage_key)
    return FileDownloadResponse(url=url, expires_in=900)